...
from .forms import (LicenseForm, LoanForm, ReleaseForm,SeriesForm,
...
def notice(request):
    ...
    data = {}
    data['totalamount'] = cust.aggregate(
        loancount=Count('loan'), t=Sum('loan__loanamount'))
    data['cust'] = cust
...
def multirelease(request, id=None):
    if request.method == 'POST':  # <- Checking for method type
        ...
        if action == 'delete':
            ...
            formset = Loan_formset(
                queryset=Loan.objects.filter(id__in=id_list))
            ...
            for loan_id in id_list:
                ...
                if not last:
                    return '1'
                Release.objects.create(releaseid=int(
                    last.id)+1, created=timezone.now(), loan_id=loan_id, interestpaid=0)
...
def home(request):
    ...
    customer['latest'] = ','.join(
        lat.name for lat in c.order_by('-created')[:5])
    customer['maxloans'] = c.filter(loan__release__isnull=True).annotate(num_loans=Count('loan'), sum_loans=Sum(
        'loan__loanamount')).values('name', 'num_loans', 'sum_loans').order_by('-num_loans')[:10]
    ...
    license['totalloans'] = series.annotate(t=Coalesce(
        Sum('loan__loanamount', filter=Q(loan__release__isnull=True)), 0))
    license['totalunreleasedloans'] = series.annotate(t=Coalesce(
        Sum('loan__loanamount', filter=Q(loan__release__isnull=False)), 0))
    ...
    loan['gold_amount'] = l.filter(
        itemtype='Gold').aggregate(t=Sum('loanamount'))
    loan['gold_weight'] = l.filter(
        itemtype='Gold').aggregate(t=Sum('itemweight'))
    loan['gavg'] = math.ceil(loan['gold_amount']['t']/loan['gold_weight']['t'])
    loan['silver_amount'] = l.filter(
        itemtype='Silver').aggregate(t=Sum('loanamount'))
    loan['silver_weight'] = l.filter(
        itemtype='Silver').aggregate(t=Sum('itemweight'))
    loan['savg'] = math.ceil(loan['silver_amount']
                             ['t']/loan['silver_weight']['t'])
    total_interest_due = 0

    loan['interestdue'] = l.aggregate(t=Sum('interest'))

    chart = (l.aggregate(gold=Sum('loanamount', filter=Q(itemtype="Gold")), silver=Sum(
        'loanamount', filter=Q(itemtype="Silver")), bronze=Sum('loanamount', filter=Q(itemtype="Bronze"))))
    fixed = []
    fixed.append(chart['gold'])
    fixed.append(chart['silver'])
    fixed.append(chart['bronze'])
    loan['chart'] = fixed
    datetimel = loans.annotate(year=Year('created')).values('year').annotate(
        l=Sum('loanamount')).order_by('year').values_list('year', 'l', named=True)
    datetime2 = unreleased.annotate(year=Year('created')).values('year').annotate(
        l=Sum('loanamount')).order_by('year').values_list('year', 'l', named=True)
...
def loan_renew(request, pk):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            # process the data in form.cleaned_data as required
            new_loanamount = loan.due() + \
                form.cleaned_data['amount'] + form.cleaned_data['interest']
            newloan = Loan(series=loan.series, customer=loan.customer, lid=Loan.objects.filter(series=loan.series).latest(
                'lid').lid+1, loanamount=new_loanamount, itemweight=loan.itemweight, itemdesc=loan.itemdesc, interestrate=loan.interestrate)
            Release.objects.create(releaseid=Release.objects.latest(
                'id').id+1, loan=loan, interestpaid=form.cleaned_data['interest'])
            newloan.save()
            # redirect to a new URL:
